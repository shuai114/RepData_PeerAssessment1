...
- First I am asked to calculate the total number of steps taken per day. Here I use `na.rm=TRUE`, so I ignore the missing values. Notice that `act$date` is already a factor variable, and I don't have to convert it to `Date` class.
...
- Then I am asked to calculate and report the mean and median of the total number of steps taken per day.
...
- First I am asked to make a time series plot of the 5-minute interval (x-axis) and the average number of steps taken, averaged across all days (y-axis). So I calculate the average number of steps taken at each 5-minute interval across all days, get the x-axis labels, and then I make the time series plot, which shows the average daily activity pattern.
...
- Now I am asked to find the 5-minute interval which contains the maximum number of steps, on average across all the days in the dataset.
...
- Now I need to make the new dataset equal to the original dataset, except with the missing data filled in. I also want to arrange it to make it look nice.
...
- Then I am asked to make a histogram of the total number of steps taken each day. This time I don't need to use `na.rm=TRUE`, because there is no missing values.
...
- First I need create a new factor variable in the dataset with two levels - "weekday" and "weekend" indicating whether a given date is a weekday or weekend day.
