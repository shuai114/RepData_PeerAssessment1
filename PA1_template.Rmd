...
```
 ...
 Thus the mean and median of the total number of steps taken per day are `r round(mean,2)` and `r median` respectively.
...
Thus the 5-minute interval requested is from `r maximalow` to `r maximaup`. It should be in the morning, as the dataset is using the 24-hour format for the time in intervals.
...
From the output, We can see that there is no missing values for the variables `date` and `interval`, and we have `r numna` missing values for the variable `steps`.
